...
import {Event, Photo} from "@/lib/types.ts";
...
export default function Dashboard({onLogout}: DashboardProps) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const data = await api.getLoveStoryData();
    ...
    const onSubmit = async (data: EventFormValues) => {
        ...
        try {
            ...
            // Xác định kiểu cho updatedEvent là `Event`
            let updatedEvent: Event;

            if (selectedEvent) {
                // Update existing event
                updatedEvent = await api.updateEvent(selectedEvent.id, eventData);
                setEvents(events.map(event =>
                    event.id === selectedEvent.id ? updatedEvent : event
                ));
            } else {
                // Add new event
                updatedEvent = await api.createEvent(eventData);
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                                    ...
                                    events.map((event) => (
                                        <li key={event.id} className="px-6 py-4 flex items-center justify-between">
                                            <div className="flex items-center">
                                                ...
                                                <div className="ml-4">
                                                    <div
                                                        className="text-sm font-medium text-gray-900">{event.title}</div>
                ...
                {section === 'map' && (
                    <div className="bg-white rounded-lg shadow p-6">
                        ...
                        <div
                            className="border border-gray-200 rounded-lg h-64 bg-gray-100 mb-4 flex items-center justify-center text-gray-400">
